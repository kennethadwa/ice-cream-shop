...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['product_id'])) {
    ...
    $imageUrl = $product['image_url']; // Use current image URL as default
    ...
    $updateQuery = "UPDATE products SET name = ?, quantity = ?, description = ?, image_url = ?, updated_at = NOW() WHERE product_id = ?";
    $stmt = $conn->prepare($updateQuery);
    $stmt->bind_param("sissi", $productName, $quantity, $description, $imageUrl, $productId);

    if ($stmt->execute()) {
        header('Location: manage_ice_cream.php?msg=Product updated successfully');
        exit();
    } else {
                                ...
                                <div class="card-body">
                                    ...
                                    <form method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="editProductName" name="productName" value="<?= $product['name'] ?>" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="editQuantity">Quantity</label>
                                            <input type="number" class="form-control" id="editQuantity" name="quantity" value="<?= $product['quantity'] ?>" required>
                                        </div>
                                        <div class="form-group">
