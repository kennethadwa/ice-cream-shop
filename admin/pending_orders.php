...
// Fetch all pending transactions including order type, ordered by the transaction ID in descending order
$query = "
    SELECT 
        t.transaction_id,
        CONCAT(u.first_name, ' ', u.last_name) AS full_name, 
        u.address AS delivery_address, 
        GROUP_CONCAT(p.name SEPARATOR ', ') AS product_names,  -- Aggregate product names
        SUM(p.price) AS total_price,  -- Sum the product prices (adjust if additional prices are involved)
    ...
    WHERE 
        t.status = 'Pending'
    GROUP BY
        t.transaction_id, u.first_name, u.last_name, u.address, ot.order_type, t.pickup_time, t.status
    ORDER BY 
                                    ...
                                    <tbody>
                                          ...
                                          if ($result->num_rows > 0) {
                                              while ($row = $result->fetch_assoc()) {
                                                  ...
                                                  echo "<td><a href='edit_pending_order.php?transaction_id={$row['transaction_id']}' class='btn btn-primary btn-sm'><i                                       class='bi bi-pencil'></i> Update</a></td>";
                                                  echo "</tr>";
                                              }
                                          } else {
